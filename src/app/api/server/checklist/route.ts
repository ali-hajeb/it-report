import { INewServer, IServerCheckList, ServerCheckList } from "@/src/lib/module/server";
...
export async function POST(req: NextRequest) {
    try {
        ...
        const server = await ServerCheckList.create(data);
        await server.populate(['location', 'server']);
...
export async function GET(req: NextRequest) {
    try {
        ...
        const count = await ServerCheckList.countDocuments({ $and: conditions });
        const servers = await ServerCheckList.find({ $and: conditions })
            .skip(parseInt(skip) * parseInt(limit))
            .limit(parseInt(limit))
            .populate(['location', 'server']);
...
export async function PATCH(req: NextRequest) {
    try {
        const body = await req.json();
        const { _id, ...updatedData } = body as IServerCheckList;

        const server = await ServerCheckList.findByIdAndUpdate(_id, updatedData, { new: true }).populate(['location', 'server']);
        return NextResponse.json({ code: 200, message: '', server }, { status: 200 });
    } catch (error) {
        return NextResponse.json({ code: 400, message: '', data: error}, { status: 400 });
    }
